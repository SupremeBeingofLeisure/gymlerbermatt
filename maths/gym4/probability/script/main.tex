...
%\title{
%\includegraphics[width=1\textwidth]{pictures/wurfelbecher}
%}
...
Mit Zufallsgeräten führt man Zufallsversuche durch, indem man die verschiedenen Aus\-gän\-ge oder Ergebnisse eines Zufallsexperiments beobachtet. Dabei versteht man unter einem Zufallsversuch einen (zumindest theoretisch) beliebig oft wiederholbaren Vorgang, dessen Ausgang sich nicht mit Sicherheit vorhersagen lässt.
\begin{itemize}
...
\item Beim \emph{Einarmigen Banditen} ist es nicht so offensichtlich, wie die einzelnen Walzen gesteuert werden. Man hat aber den Eindruck, dass sie zufällig über Gewinn und Verlust entscheiden.
\end{itemize}

\begin{cdef}[Stichprobenraum]{}
Die Menge aller möglichen Versuchsausgänge $\go_1, \go_2, \dots, \go_n$ eines Zufallsversuchs bezeichnet man als \emph{Stichprobenraum} $\gO$.
\end{cdef}

\begin{bsps}
\ \\[-4ex]
\begin{description}
\item Münze: $\go_1=$Kopf, $\go_2=$Zahl, also $\gO=\set{\text{Kopf, Zahl}}$
\item Würfel: $\gO=\set{1,2,3,4,5,6}$
\end{description}
\end{bsps}

\begin{ueb}[Stichprobenraum notieren]
Bestimme einen geeigneten Stichprobenraum für den Versuch
\begin{enumeratea}
\item Mit der Spitze eines Bleistifts wird blindlings auf
eine Textseite eines Buches getippt; der getroffene Buchstabe wird notiert.
\item Bei Familien mit drei Kindern wird festgestellt, ob
die Kinder Jungen oder Mädchen sind.
\end{enumeratea}
\end{ueb}

\subsubsection{Ereignisse}
Wenn man beim Roulette auf \glqq impair\grqq\ setzt, ist man nicht so sehr daran interessiert, in welches Zahlenfach die Kugel fällt. Wichtig ist nur, ob das zufällige Ereignis eintritt, dass die ausgeloste Zahl ungerade ist, also die Kugel in eines der Fächer $1,3,5, \dots, 33, 35$ fällt. Diese gewinnbringenden Ausfälle bilden eine Teilmenge $\mA$ des Stichprobenraumes $\gO=\set{0,1,2,3,\dots,35,36}$.
...
\begin{cdef}[Ereignis]{}
Jede Teilmenge $\mA\subset\gO$ ist ein \emph{Ereignis}.
\end{cdef}

Endet eine Durchführung des Zufallversuchs mit dem Ausgang $\go\in\mA$, so sagt man: Das Ereignis $\mA$ ist eingetreten.	 Ist $\go\not\in\mA$, so sagt man: Das Ereignis $\mA$ ist nicht eingetreten oder das \emph{Gegenereignis} $\overline{\mA} = \gO\setminus\mA$ ist eingetreten.

Unter den Ereignissen eines Zufallsversuchs gibt es zwei Sonderfälle, nämlich $\mA=\gO$ und $\mA=\emptyset$. Da $\gO$ alle möglichen Ereignisse enthält, tritt $\gO$ bei jeder Durchführung des Versuchs ein. Man bezeichnet $\gO$ deshalb auch als das \emph{sichere Ereignis}. Da die leere Menge überhaupt kein Element enthält, kann dieses Ereignis bei keiner Durchführung des Versuchs eintreten. Man bezeichnet $\emptyset$ deshalb auch als das \emph{unmögliche Ereignis}. Beachten Sie $\overline{\gO}=\gO\setminus\gO=\emptyset$ und  $\overline{\emptyset}=\gO\setminus\emptyset=\gO$, d.h. das Gegenereignis vom sicheren Ereignis ist das unmögliche Ereignis und umgekehrt.
Sind $\mA$ und $\mB$ zwei Ereignisse desselben Zufallsversuchs und endet die Durchführung des Zufallversuchs mit dem Ausgang $\go$, so sagt man,
\begin{itemize}
\item falls $\go\in\mA\cup\mB$: \glqq Ereignis $\mA$ \emph{oder} $\mB$ ist eingetreten\grqq,
\item falls $\go\in\mA\cap\mB$: \glqq Ereignis $\mA$ \emph{und} $\mB$ ist eingetreten\grqq.
\end{itemize}
Zwei Ereignisse $\mA$ und $\mB$ heissen \emph{unvereinbar} genau dann, wenn sie nicht zugleich eintreten können, wenn also gilt $\mA\cap\mB=\emptyset$. Gegenereignisse $\mA$ und $\overline{\mA}$ sind stets unvereinbar.

\begin{ueb}[Ereignisse beschreiben]
...
Auf Grund dieses Beispiels liegt es nahe, jedem Ausgang $\go$ eines Zufallversuchs eine Wahrscheinlichkeit $P(\go)$ zuzuordnen. Diese Wahrscheinlichkeit steht für den festen Wert, auf den sich die relative Häufigkeit des betreffenden Ausgangs konzentriert, wenn der Versuch oft wiederholt wird. Beim Reissnagel wurden diese Werte $P(1)\approx0.625$ und $P(0)\approx0.375$ empirisch durch Experimente ermittelt. Bei anderen Zufallsgeräten kann man die Wahrscheinlichkeit oft auf Grund ihrer Symmetrie angeben.

\begin{bem}
Die Bezeichnung $P$ erinnert an probability.
\end{bem}
...
\subsection{Erste Definition einer Wahrscheinlichkeit}

Nachdem jedem Ausgang $\go$ eines Zufallversuchs auf plausible Weise ein Wert als Wahrscheinlichkeit zugeordnet werden konnte, liegt es nahe, entsprechend die Wahrscheinlichkeit $P(\go)$ für ein Ereignis $\go$ zu definieren.

\begin{cdef}[Wahrscheinlichkeit eines Ereignisses]{}
$P(\go)$ ist gleich der Summe der Wahrscheinlichkeiten aller Ausgänge in $\mA$.
\end{cdef}
...
\begin{ueb}[Geburten-Faust-Regel]
...
$P$ eine Funktion, die jedem Ereignis $\mA$ eines Stichprobenraumes $\gO$ genau eine reelle Zahl $P(\mA)$ zuordnet, so heisst $P$ eine \emph{Wahrscheinlichkeitsfunktion} und jeder Funktionswert $P(\mA)$ die \emph{Wahrscheinlichkeit} von $\mA$, genau dann, wenn folgende (Kolmogorow-)Axiome erfüllt sind:
\begin{itemize}
\item $P(\mA)\geq0, \forall\mA\subset\gO$ \hfill(Positivität)
\item $P(\gO):=1$ \hfill(Normierung)
\item $\forall\mA,\mB\subset\gO$ mit $\mA\cap\mB=\emptyset\implies P(\mA\cup\mB)=P(\mA)+P(\mB)$ \hfill(Additivität)
\end{itemize}
\end{cdef}
...
$$P(\overline{\mA})=1-P(\mA).$$
\end{csatz}
\begin{proof}[Beweis]
Wegen $\mA\cap\overline{\mA}=\emptyset$ und $\mA\cup\overline{\mA}=\gO$ gilt
$$1=P(\gO)=P(\mA\cup\overline{\mA})=P(\mA)+P(\overline{\mA}),$$
also $P(\overline{\mA})=1-P(\mA)$.
\end{proof}

\begin{bsp}
Ein illustratives Beispiel für eine Anwendung ist die Suche nach der Antwort auf die Frage:
\begin{quote}
Wie gross ist die Wahrscheinlichkeit in $10$ würfen mindestens einmal eine $6$ zu werfen?
\end{quote}
...
Einfacher geht es jetzt über das Gegenereignis von \glqq mindestens $1$-mal eine $6$ zu werfen\grqq; das ist nämlich \glqq nie eine $6$ in $10$ Würfen\grqq\ zu erzielen. Es ist
$$P(\text{keine $6$ in $10$ Würfen})=\left(\frac{5}{6}\right)^{10}\approx0.1615.$$
Nun kann man mit obigem Satz einfach schliessen, dass die Wahrscheinlichkeit \glqq in $10$ Würfen mindestens $1$-mal eine $6$\grqq\ zu würfeln
\begin{align*}
...
\begin{bem}
Die Beziehung aus obigem Satz wird sehr oft benötigt, da in vielen Fällen die Wahrscheinlichkeit des Gegenereignisses $\overline{\mA}$ leichter zu berechnen ist als die Wahrscheinlichkeit des Ereignisses $\mA$.
\end{bem}

\begin{csatz}[günstig durch möglich]{}
Hat ein Zufallsversuch $n$ mögliche Ausfälle $\go_1, \go_2, \go_3, \dots, \go_n$ und ist jeder dieser Ausfälle gleich wahrscheinlich, also $P(\go_i)=\frac{1}{n}$, so gilt für ein Ereignis $\mA$
$$P(\mA)=\frac{1}{n}+\frac{1}{n}+\dots+\frac{1}{n}=\frac{\card(\mA)}{\card(\gO)}.$$
\end{csatz}
\begin{proof}[Beweis]
...
In obigem Fall gilt also für die Wahrscheinlichkeit eines Ereignisses $\mA$: Anzahl der günstigen Fälle für $\mA$ geteilt durch Anzahl der möglichen Fälle $\gO$, oder kurz
$$P(\mathbb{A}):=\frac{|\mathbb{A}|}{|\Omega|}=\frac{g}{m}.$$
\end{cdef}

\begin{bsps}
Würfeln mit einem guten Spielwürfel, Ziehen von Losen aus einer gut durchmischten Urne, Ziehen von Karten aus einem gut durchmischten Kartenspiel etc.
\end{bsps}
...
\begin{ueb}[blind oder taub?]
Die Wahrscheinlichkeit, dass jemand blind bzw. taub ist, beträgt $0.0085$ bzw. $0.005$. Beide Gebrechen kombiniert kommen mit der Wahrscheinlichkeit $0.0006$ vor. Wie gross ist die Wahrscheinlichkeit, dass man blind oder taub ist?
\end{ueb}
...
\begin{ueb}[Tankstelle Warteschlange]
...
Hier hängt die zweite Stufe nicht vom Ausgang der ersten Stufe ab. Der zugehörige Stichprobenraum ist $\gO=\set{rr, rg, gr, gg}$.
...
\begin{ueb}[Von Neumann Trick für gefälscht Münze]
...
Sei $\mB$ ein Ereignis mit $P(\mB) > 0$. Die Wahrscheinlichkeit des Eintretens des Ereignisses $\mA$ unter der Bedingung, dass das Ereignis $\mB$ bereits eingetreten ist, oder kurz, die \emph{bedingte Wahrscheinlichkeit} von $\mA$ unter der Bedingung $\mB$, $P(\mA|\mB)$, wird definiert durch
$$P(\mA|\mB):=\frac{P(\mA\cap\mB)}{P(\mB)}.$$
\end{cdef}
...
Eine Funktion, deren Definitionsmenge der Stichprobenraum $\gO$ ist und deren Wertemenge die reellen Zahlen sind
$$X:\gO \to \mR$$
...
\begin{figure}
  ...
  \end{center}
\caption{Zoom in die alte 10\,DM Note mit Carl Friedrich Gauss}
\end{figure}
...
Ein Zufallsversuch mit genau zwei Ausgängen, also mit dem Stichprobenraum $\gO=\{\text{Erfolg, Misserfolg}\}$ oder kurz $\set{1,0}$, heisst \emph{Bernoulli-Versuch}.
...
Berechne nun die Wahrscheinlichkeit $\ga_p$ dafür, dass wir bei $n$ Versuchen ein Ereignis aus dem kritischen Bereich $K$ erhalten, falls $H_0$ wahr ist. Im Falle von Nicole haben wir also $n=5$, $K=\set{3,4,5}$ und $p=0.4$.
\end{ueb}
...
Will Nicole mit der Versuchsanordnung von 5 Freiwürfen im Wettkampf zeigen, dass sie ein Wettkampftyp ist, dann entsteht das folgende Problem. Um die Wahrscheinlichkeit eines Fehlers erster Art unter 5\% zu bringen (Signifikanzniveau), muss Nicole wirklich gut treffen, damit wir die Nullhypothese $H_0$ ablehnen. Sie muss so gut treffen, dass die Wahrscheinlichkeit eines Fehlers zweiter Art sehr gross wird. Somit hat es eine wahre Alternativhypothese sehr, sehr schwer\dots
\marginnote{
\qrcode{
...
\begin{ueb}[Nicole wirft 25 mal]\label{ueb25}
...
\begin{ueb}[Dreisteller mit 6 Ziffern]
\ \\[-4ex]
\begin{enumeratea}
...
Bei der Aufgabe, die Buchstaben des Wortes \texttt{SISSI} auf verschiedene Arten anzuordnen, handelt es sich um eine Permutation von fünf Elementen, die aber nicht alle verschieden sind. In diesem Fall gibt es natürlich nicht $5!$ Permutationen.
