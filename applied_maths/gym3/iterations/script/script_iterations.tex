...
Wir betrachten einige Beispiele.
\begin{itemize}
\item $$f(x)=\frac{x}{2},\quad x_0=3\dots$$ Die Folge ist konvergent gegen $0$.
\item $$f(x)=3x,\quad x_0=1\dots$$ Die Folge divergiert.
\item $$f(x)=x,\quad x_0\dots$$ Hier haben wir lauter Fixpunkte.
\item $$f(x)=x+c,\quad x_0\dots$$ Wir haben $f^n(x)=x_0+n\cdot c$, was im Allgemeinen divergiert.
>>>>>>> Stashed changes
...
\end{bsps}
=======
\item Gilt $|x_0|>1$, so divergiert die entsprechende Folge.
\end{itemize}
\end{itemize}
\end{bsp}
>>>>>>> Stashed changes
...
\begin{tikzpicture}[node distance=1.5cm, every edge/.style={link}]
  ...
  \node[entity,top color=white, bottom color=seagreen!30, 
                            draw=seagreen!50!black!100, drop shadow] (p) {Susceptible};

  \node[entity,top color=white, bottom color=firebrick!30, 
                            draw=firebrick!50!black!100, drop shadow] (c) [right =of p] {Infected}; %edge [<-] (p);
                            \draw[->] (p) -- (c);
  
  \node[entity,top color=white, bottom color=darkblue!30, 
                            draw=darkblue!50!black!100, drop shadow] (m) [right =of c] {Immune} ;
                            \draw[->] (c) -- (m);
